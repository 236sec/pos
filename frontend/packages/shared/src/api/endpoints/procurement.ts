...
export function createProcurementEndpoints(client: ApiClient) {
  return {
    getSuppliers: () => client.get<Supplier[]>("/suppliers"),

    createPurchaseOrder: (data: Record<string, unknown>) =>
      client.post<PurchaseOrder>("/purchase-orders", data),

    getPurchaseOrders: () => client.get<PurchaseOrder[]>("/purchase-orders"),
...
export type ProcurementEndpoints = ReturnType<
  typeof createProcurementEndpoints
>;
